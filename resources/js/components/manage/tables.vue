            ...
            <td class="text-xs-right">
              <span>
                <v-btn
                  ...
                  round
                  color="error"
                  @click.native="delete(props.item.table_number), table_number_log(props.item.table_number)"
                >Delete</v-btn>
              </span>
            </td>
          </tr>
...
export default {
  ...
  methods: {
    ...
    delete(table_number) {
      ...
      axios
        .delete("/api/tables/delete", this.table, config)
        .then(response => {
          this.totalTables--;
    ...
    isUserAWorker(user) {
      if (user.type == "manager") {
        ...
        this.$toasted.error(
          "You are not authorized to see this page! Managers only.",
          {
            icon: "error_outline",
            onClick: (e, toastObject) => {
              toastObject.goAway(0);
